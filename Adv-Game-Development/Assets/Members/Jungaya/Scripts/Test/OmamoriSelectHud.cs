...
/*
    今選んでいるお守りを、画面の右下に大きく表示する HUD
    OmamoriSelector.Current を毎フレーム見て、種類ごとの色で四角（Image）をぬって、
    種類の名前を Text に出す。左上の小さいデバッグ表示（OnGUI）より見やすい

    色と名前は Inspector から変えられる（OmamoriType の enum の順番: 健康/学業成就/厄除け安全/縁結び/金運）
    ちゃんとした選ぶUI（#9）ができたら、この HUD ごと入れかえればいい
*/
public class OmamoriSelectHud : MonoBehaviour
{
    ...
    OmamoriType _last = (OmamoriType)(-1); // 最初に必ず反映されるように、enum にない値からスタートする

    void Start()
    {
        // Text の font が入っていないと何も描かれないので、組みこみのフォントを入れておく
    ...
    void Apply()
    {
        ...
        int i = (int)_last;
        // 色の正しい値は OmamoriPalette（v3 §3）。入っていないときだけ予備の配列を使う
