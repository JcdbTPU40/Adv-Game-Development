...
{
    /// <summary>
    /// 笑顔の伝播（#56）の確認シーンを組み立てる Editor 拡張。
    ///
    /// 置くのは「地面・カメラ・入力・照準・スコア・足元の円・確認進行」だけ。
    /// 客は <see cref="SmilePropagationVerifier"/> が実行時に並べるので、シーンを作り直さなくても
    /// F5（密集）／F6（遠方）／F7（黒客混在）で配置を切り替えられる。
    ///
    /// 確認の手順は Docs/56_笑顔の伝播.md にある。
    /// </summary>
    public static class SmilePropagationSceneBuilder
    {
        const string BuildMenuPath = "Toufuku/Rescue/Build Smile Propagation Scene";
        const string OpenMenuPath = "Toufuku/Rescue/Open Smile Propagation Scene";
        ...
        const string ScenePath = SceneFolder + "/SmilePropagationTest.unity";
        ...
        [MenuItem(BuildMenuPath, priority = 252)]
        public static void Build()
        {
            ...
            Debug.Log(
                $"[#56] 確認シーンを生成しました: {ScenePath}\n" +
                "Play して F5=密集／F6=遠方／F7=黒客混在。1 キーで色を選びクリックで投げるか、F9 で対象を強制救済。");
        }

        [MenuItem(OpenMenuPath, priority = 253)]
        ...
        static void BuildScene(Material groundMat, ScoreBonusTable bonusTable)
        {
            ...
            // ── 確認の進行（密集／遠方／黒客混在）──
            var verifyGo = new GameObject("SmilePropagationVerifier");
            var verifier = verifyGo.AddComponent<SmilePropagationVerifier>();
        ...
        /// <summary>加点数値表（付録B B-2）。無ければ既定値（伝播 +20 / 4人）で作る。</summary>
        static ScoreBonusTable EnsureBonusTable()
        {
            ...
            Debug.Log($"[#56] 加点数値表を作りました: {BonusTablePath}（伝播 +20 / 上限 4人 = 最大 +80）");
            return table;
        }

        /// <summary>
        /// 地面のマテリアル。#55 の確認シーンと同じものを共用する。
        /// 既にあるものには手を触れない（作り直すたびにマテリアルの差分が出るのを避ける）。
        /// </summary>
        ...
        static void SetObject(Object target, string propertyName, Object value)
        {
            ...
            if (property == null)
            {
                Debug.LogWarning($"[#56] {target.GetType().Name}.{propertyName} が見つかりません", target);
                return;
            }
